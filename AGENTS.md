...
- Lab deliverables live under `labs/Lab_#/Lab_#_Kendall_Davies/` (e.g., `labs/Lab_4/Lab_4_Kendall_Davies/`) and each folder keeps its own notebook, helper scripts, and figures; mirror this layout when adding new experiments so datasets and outputs stay co-located.
- Datasets are stored in `datasets/` subdirectories within each lab directory (e.g., `labs/Lab_2/Lab_2_Kendall_Davies/datasets/`).
- Shared references, starter templates, and lecture material are stored in `resources/Week_*/` with `examples/` and `templates/` subdirectories; never edit originals—copy into the relevant lab directory first.
...
- `uv run python labs/Lab_4/Lab_4_Kendall_Davies/advanced_text_generation.py` — exercise the reusable RNN evaluation helpers outside the notebook.
...
- Notebooks must be named `Lab_#_<YourName>.ipynb`; auxiliary scripts mirror the notebook stem (e.g., `labs/Lab_4/Lab_4_Kendall_Davies/text_cleaner.py`) so reviewers can trace provenance quickly.
...
## Testing Guidelines
- Each lab should include at least one verification cell that reuses held-out data (e.g., `datasets/fashion_mnist_test_features.npy` in `labs/Lab_3/Lab_3_Kendall_Davies/`) and records metrics inside the notebook plus a concise note in `training_log.txt`.
...
- Attach screenshots or PNG exports for plots placed under the corresponding lab folder (`labs/Lab_5/Lab_5_Kendall_Davies/LSTM_out.png`) so reviewers can validate visuals without re-running notebooks.
